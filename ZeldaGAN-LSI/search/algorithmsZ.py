'''
Illumination algorithms used.
Also contains unused and unmodified algorithms from MarioGAN.
'''
...
from util.gan_generatorZ import *
    ...
    def return_evaluated_individual(self, ind):
        ...
        if self.individuals_evaluated % record_frequency == 0:
            ...
            if(len(elites)!=0):
                ...
                for x in elites:
                    currElite=[x.ID]
                    currElite+=self.all_records.loc[x.ID][['completable']].tolist()
                    currElite+=self.all_records.loc[x.ID][self.bc_names].tolist()
                    rowData.append(currElite)
                wr = csv.writer(logFile, dialect='excel')
                wr.writerow(rowData)
